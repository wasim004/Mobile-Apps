...
class FeedViewController: UITableViewController {
    ...
    func retrieveTweets() { tweets?.removeAllObjects()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cachedImage = image {
            ...
            operationQueue.addOperation() {
                ...
                do {
                    if
                        let imageData : Data = try Data(contentsOf: imageURL!) { let image = UIImage(data: imageData) as UIImage?
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "ComposeTweet") {
            ...
            if let path : IndexPath = self.tableView.indexPathForSelectedRow { if let tweetData = self.tweets?.object(at: path.row) {
